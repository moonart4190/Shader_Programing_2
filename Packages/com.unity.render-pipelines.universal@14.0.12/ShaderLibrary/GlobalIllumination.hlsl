...
#if defined (MLS_TEXTURE_ARRAYS_ON) && defined (MLS_LIGHTMAPS_BLENDING_ON)
    mls_AdditionalObjectData = float4(mls_AdditionalObjectData.x, mls_AdditionalObjectData.y, GetMainLightShadowStrength(), 0);
#endif
...
half3 CalculateIrradianceFromReflectionProbes(MLS_CALCULATE_IRRADIANCE_ARGS)
{
    ...
    if (weightProbe0 > 0.01f)
    {
        ...
        #if defined (MLS_REFLECTIONS_BLENDING_ON)
            half4 encodedIrradiance = half4(BlendTwoCubeTextures(0, reflectVector0, mip, unity_SpecCube0_ProbePosition.xyz));
        #else
...
#endif // _REFLECTION_PROBE_BOX_PROJECTION
        #if defined (MLS_REFLECTIONS_BLENDING_ON)
            half4 encodedIrradiance = half4(BlendTwoCubeTextures(1, reflectVector1, mip, unity_SpecCube1_ProbePosition.xyz));
        #else
    ...
    {
        #if defined (MLS_REFLECTIONS_BLENDING_ON)
            half4 encodedIrradiance = half4(BlendTwoCubeTextures(2, reflectVector, mip, 0));
        #else
...
#if !USE_FORWARD_PLUS
half3 CalculateIrradianceFromReflectionProbes(MLS_CALCULATE_IRRADIANCE_ARGS_LEGACY)
{
    return CalculateIrradianceFromReflectionProbes(MLS_CALCULATE_IRRADIANCE_ARGS_LEGACY_USE);
}
#endif

half3 GlossyEnvironmentReflection(MLS_GLOSSY_ENVIRONMENT_ARGS_V1)
{
#if !defined(_ENVIRONMENTREFLECTIONS_OFF)
    half3 irradiance;

#if defined(_REFLECTION_PROBE_BLENDING) || USE_FORWARD_PLUS
    irradiance = CalculateIrradianceFromReflectionProbes(MLS_CALCULATE_IRRADIANCE_ARGS_USE);
#else
#ifdef _REFLECTION_PROBE_BOX_PROJECTION
    ...
    half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness);
    #if defined (MLS_REFLECTIONS_BLENDING_ON)
         half4 encodedIrradiance = half4(BlendTwoCubeTextures(0, reflectVector, mip, unity_SpecCube0_ProbePosition.xyz));
    #else
...
#if !USE_FORWARD_PLUS
half3 GlossyEnvironmentReflection(MLS_GLOSSY_ENVIRONMENT_ARGS_V2)
{
    return GlossyEnvironmentReflection(MLS_GLOSSY_ENVIRONMENT_ARGS_V2_USE);
}
#endif

half3 GlossyEnvironmentReflection(MLS_GLOSSY_ENVIRONMENT_ARGS_V3)
{
#if !defined(_ENVIRONMENTREFLECTIONS_OFF)
    half3 irradiance;
    half mip = PerceptualRoughnessToMipmapLevel(perceptualRoughness);
...
half3 GlobalIllumination(MLS_GLOBAL_ILLUMINATION_ARGS_V1)
{
    ...
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfData.perceptualRoughness, 1.0h, normalizedScreenSpaceUV, mls_AdditionalObjectData);
    #else
    ...
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        half3 coatIndirectSpecular = GlossyEnvironmentReflection(reflectVector, positionWS, brdfDataClearCoat.perceptualRoughness, 1.0h, normalizedScreenSpaceUV, mls_AdditionalObjectData);
    #else
...
#if !USE_FORWARD_PLUS
half3 GlobalIllumination(MLS_GLOBAL_ILLUMINATION_ARGS_V2)
{
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        return GlobalIllumination(brdfData, brdfDataClearCoat, clearCoatMask, bakedGI, occlusion, positionWS, normalWS, viewDirectionWS, float2(0.0f, 0.0f), mls_AdditionalObjectData);
    #else
...
half3 GlobalIllumination(MLS_GLOBAL_ILLUMINATION_ARGS_V3)
{
    const BRDFData noClearCoat = (BRDFData)0;
    
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        return GlobalIllumination(brdfData, noClearCoat, 0.0, bakedGI, occlusion, positionWS, normalWS, viewDirectionWS, 0, mls_AdditionalObjectData);
    #else
        return GlobalIllumination(brdfData, noClearCoat, 0.0, bakedGI, occlusion, positionWS, normalWS, viewDirectionWS, 0);
    #endif
}

half3 GlobalIllumination(MLS_GLOBAL_ILLUMINATION_ARGS_V4)
{
    half3 reflectVector = reflect(-viewDirectionWS, normalWS);
    half NoV = saturate(dot(normalWS, viewDirectionWS));
    half fresnelTerm = Pow4(1.0 - NoV);
    ...
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        half3 indirectSpecular = GlossyEnvironmentReflection(reflectVector, brdfData.perceptualRoughness, half(1.0), mls_AdditionalObjectData);
    #else
    ...
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        half3 coatIndirectSpecular = GlossyEnvironmentReflection(reflectVector, brdfDataClearCoat.perceptualRoughness, half(1.0), mls_AdditionalObjectData);
    #else
...
half3 GlobalIllumination(MLS_GLOBAL_ILLUMINATION_ARGS_V5)
{
    const BRDFData noClearCoat = (BRDFData)0; 
    
    #if defined (MLS_TEXTURE_ARRAYS_ON)
        return GlobalIllumination(brdfData, noClearCoat, 0.0, bakedGI, occlusion, normalWS, viewDirectionWS, mls_AdditionalObjectData);
    #else
