...
namespace MagicLightmapSwitcher
{
    public class Switching
    {
        ...
        private static IEnumerator LoadLightingDataAsync(StoredLightmapData lightmapData, LayerMask blendingModules)
        {
            if ((blendingModules & (1 << 0)) > 0)
            {
...
#if BAKERY_INCLUDED
            if ((blendingModules & (1 << 5)) > 0)
#else
            if ((blendingModules & (1 << 4)) > 0)
#endif
            {
        ...
        private static void LoadRenderersData(StoredLightmapData lightmapData)
        {
            ...
            {
                var staticRenderer = renderers[i].GetComponent<MLSStaticRenderer>();                

                if (staticRenderer != null)
                {
                    var affectedObject = Blending._resultStaticAffectedObjects.Find(item => item.objectId == staticRenderer.scriptId);
                    ...
                    if (lightmapData.rendererDataDeserialized[staticRenderer.scriptId] is StoredLightmapData.RendererData rendererData)
                    {
                        ...
                        {
                            if (rendererData.rendererShaderProperties.Count > 0)
                            {
                                for (int p = 0; p < rendererData.rendererShaderProperties.Count; p++)
                                {
                                    switch (rendererData.rendererShaderProperties[p].type)
                                    {
                                        ...
                                        case ShaderPropertyType.Range:
                                            affectedObject.SetShaderFloat(
                                                rendererData.rendererShaderProperties[p].name,
                                                rendererData.rendererShaderProperties[p].floatValue);
                                            break;
                                        case ShaderPropertyType.Color:
                                            affectedObject.SetShaderColor(
                                                rendererData.rendererShaderProperties[p].name,
                                                rendererData.rendererShaderProperties[p].colorValue);
                                            break;
                                        case ShaderPropertyType.Texture:
                                            affectedObject.SetShaderTexture(
                                                rendererData.rendererShaderProperties[p].name,
                                ...
                                affectedObject.ApplyPropertyBlock();
                            }
                        }
                    }
                }
        ...
        public static void LoadLightingData(MagicLightmapSwitcher parent, int lightmapDataIndex, LoadMode loadMode = LoadMode.Synchronously)
        {
            ...
            {
#if UNITY_EDITOR
                loadLightingDataRoutine = LoadLightingDataAsync(dataToLoad, parent.currentLightmapScenario.blendingModules);
                EditorApplication.update += LoadLightingDataIteratorUpdate;
#else

                parent.StartCoroutine(LoadLightingDataAsync(dataToLoad, parent.currentLightmapScenario.blendingModules)); 
#endif
            }
            else
            {
                if ((parent.currentLightmapScenario.blendingModules & (1 << 0)) > 0)
                {
                ...
                if ((parent.currentLightmapScenario.blendingModules & (1 << 1)) > 0)
                {
                    LoadReflectionProbesData(dataToLoad);
                }

                if ((parent.currentLightmapScenario.blendingModules & (1 << 2)) > 0)
                {
                    LoadEnvironmentSettings(dataToLoad);
                }

#if BAKERY_INCLUDED
                if ((parent.currentLightmapScenario.blendingModules & (1 << 5)) > 0)
#else
                if ((parent.currentLightmapScenario.blendingModules & (1 << 4)) > 0)
#endif
                {
...
#if BAKERY_INCLUDED
                if ((parent.currentLightmapScenario.blendingModules & (1 << 6)) > 0)
#else
                if ((parent.currentLightmapScenario.blendingModules & (1 << 5)) > 0)
#endif
                {
                ...
                if ((parent.currentLightmapScenario.blendingModules & (1 << 3)) > 0)
                {
