...
Shader "Magic Lightmap Switcher/URP/Skybox/Panoramic" {
Properties {
...
SubShader {
    ...
    Pass {
        ...
        #pragma multi_compile _MLS_SKYBOX_BLENDING_ON _MLS_SKYBOX_BLENDING_OFF

        sampler2D _MLS_Sky_Cubemap_Blend_From;
        sampler2D _MLS_Sky_Cubemap_Blend_To;

        float4 BlendTwoTextures(float2 tc)
        {
            float4 textureFrom;
            float4 textureTo;
            float blendFactor;

            textureFrom = tex2D (_MLS_Sky_Cubemap_Blend_From, tc);
            textureTo = tex2D (_MLS_Sky_Cubemap_Blend_To, tc);
        ...
        fixed4 frag (v2f i) : SV_Target
        {
            ...
            #ifdef _MLS_SKYBOX_BLENDING_ON
                tex = BlendTwoTextures(tc);
            #else
                tex = tex2D (_MainTex, tc);
            #endif
