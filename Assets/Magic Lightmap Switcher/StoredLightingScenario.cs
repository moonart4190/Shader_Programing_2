...
namespace MagicLightmapSwitcher
{
    ...
    public class StoredLightingScenario : ScriptableObject
    {
        ...
        public class LightmapData
        {
            ...
            public Vector2 reflectionsBlendingRange = new Vector2(0, 1);
            public Vector2 lightmapBlendingRange = new Vector2(0, 1);
        ...
        public ReorderableList reorderableStoredLightmapsData;
        ...
        public string SelfTest()
        {
            ...
            var prevStaticObjectsCount = 0;
            var staticObjectsCount = 0;
            ...
            for (int i = 0; i < blendableLightmaps.Count; i++)
            {
                ...
                if (blendableLightmaps[i] != null)
                {
                    ...
                    staticObjectsCount = blendableLightmaps[i].lightingData.rendererDataDeserialized.Count;

                    if (i > 0)
                    {
                        if (staticObjectsCount != prevStaticObjectsCount)
                        {
                            preset2 = $"[{blendableLightmaps[i].lightingData.dataName}] Objects count - {staticObjectsCount}";
                            errorCode = 1;
                            break;
                        }
                    }

                    preset1 = $"[{blendableLightmaps[i].lightingData.dataName}] Objects count - {staticObjectsCount}";
                    prevStaticObjectsCount = blendableLightmaps[i].lightingData.rendererDataDeserialized.Count;
                }
                else
                {
                    errorCode = 0;
            ...
            {
                case 0:
                    result = "An error was detected in the stored lightmap data. " +
                        "Try reassigning the data in the blending queue.";
                    break;
                case 1:
                    result = $"Not all presets contain the same number of static objects." +
                        $"This will result in broken baked lighting, as the lightmap atlases are not identical. \r\n" +
                        $"Check your presets and make sure that you don't enable/disable static objects between presets. " +
                        $"If you need to \"hide\" some objects in different lighting options, then a good way to do this is to move them out of view.\r\n\r\n" +
                        $"Different number of objects found in presets:\r\n" +
                        $"<b>{preset1}</b>\r\n" +
                        $"<b>{preset2}</b>";
                    break;
                default:
                ...
                for (int j = 0; j < collectedCustomBlendableDatas[i].blendableFloatFieldsDatas.Count; j++)
                {
                    ...
                    switch (collectedCustomBlendableDatas[i].blendableFloatFieldsDatas[j].blendingSynchMode)
                    {
                        ...
                        case CollectedCustomBlendableData.BlendingSynchMode.WithLightmaps:
                            ...
                            break;
                        case CollectedCustomBlendableData.BlendingSynchMode.InstantSwitching:
                            collectedCustomBlendableDatas[i].blendableFloatFieldsDatas[j].sourceField.SetValue(collectedCustomBlendableDatas[i].sourceScript, toData.fieldValue);
                            break;
                    }
                }
        ...
        public void RebuildSourceList()
        {
            if (cyclic)
            {
                var addCount = startValues.Count - 1;
            ...
            {
                var totalCount = startValues.Count - 1;
                var removeCount = startValues.Count / 2;

                for (int i = totalCount; i > removeCount; i--)
                {
                    startValues.RemoveAt(i);
                    blendableLightmaps.RemoveAt(i);
                }
        ...
        public void BuildReorderableList()
        {
            ...
            reorderableStoredLightmapsData = new ReorderableList(scenarioSerializedObject, scenarioSerializedObject.FindProperty("blendableLightmaps"), true, true, true, true);

            reorderableStoredLightmapsData.drawHeaderCallback = (Rect rect) =>
            {
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x + 35, rect.y, 150, rect.height),
                    "Short Name");

                EditorGUI.LabelField(
                    new Rect(rect.x + 115, rect.y, rect.width - 150, rect.height),
            ...
            reorderableStoredLightmapsData.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
            {
                var element = reorderableStoredLightmapsData.serializedProperty.GetArrayElementAtIndex(index);
                rect.y += 2;

                if (scenarioSerializedObject.FindProperty("newAdded").boolValue && index == reorderableStoredLightmapsData.serializedProperty.arraySize - 1)
                {
                    element.FindPropertyRelative("reflectionsBlendingRange").vector2Value = new Vector2(0, 1);
                    element.FindPropertyRelative("lightmapBlendingRange").vector2Value = new Vector2(0, 1);
                    scenarioSerializedObject.FindProperty("newAdded").boolValue = false;
                }
                ...
                EditorGUI.LabelField(
                        new Rect(rect.x + 20, rect.y, 100, EditorGUIUtility.singleLineHeight),
                        asset == null ? "-No Asset-" : asset.dataName);

                EditorGUI.PropertyField(
                    new Rect(rect.x + 200, rect.y, rect.width - 350, EditorGUIUtility.singleLineHeight),
                    element.FindPropertyRelative("lightingData"),
                    GUIContent.none);

                if (index > 0 && index < reorderableStoredLightmapsData.serializedProperty.arraySize - 1)
                {
                ...
                {
                    if (index == reorderableStoredLightmapsData.serializedProperty.arraySize - 1)
                    {
                        EditorGUI.LabelField(
                            new Rect(rect.x + 100 + (rect.width - 230), rect.y, 50, EditorGUIUtility.singleLineHeight),
                            "1");
                ...
                if (GUI.Button(new Rect(rect.x + (rect.width - 75), rect.y, 75, EditorGUIUtility.singleLineHeight), "Load"))
                {
                    ...
                    {
                        Blending.Blend(lightmapSwitcher, startValues[index], this);
                    }
            ...
            reorderableStoredLightmapsData.onRemoveCallback = (ReorderableList l) =>
            {
            ...
            reorderableStoredLightmapsData.onAddCallback = (ReorderableList l) =>
            {
                ...
                scenarioSerializedObject.FindProperty("newAdded").boolValue = true;
            };
            
            reorderableStoredLightmapsData.onReorderCallback = (ReorderableList l) =>
            {
