...
namespace MagicLightmapSwitcher
{
    public static class MLSLightmapDataStoring
    {
        ...
        private static IEnumerator BakeAndStoreLightmapQueue(MagicLightmapSwitcher activeSwitcherInstance, bool createLightmapScenario)
        {
            ...
            activeSwitcherInstance.globalDataArraysInitialized = false;
            ...
            for (int i = 0; i < activeSwitcherInstance.lightingPresets.Count; i++)
            {
                ...
                switch (currentLightmapper)
                {
                    ...
                    case MagicLightmapSwitcher.Lightmapper.BakeryLightmapper:
                        ...
                        if (activeSwitcherInstance.useTextureArrays)
                        {
            ...
            if (!stopBakingQueue)
            {
                ...
                {
                    if (activeSwitcherInstance.availableScenarios.Count > 0 && scenarioToChange > 0)
                    {
                        int removeIndex = scenarioToChange - 1;
                        RemoveStoredLightingScenario(removeIndex);
                        CreateLightmapScenario(activeSwitcherInstance, "New Lightmap Scenario_" + (scenarioToChange - 1).ToString(), tempStoredLightmapDatas);
                    }
                    else
                    {
        ...
        public static IEnumerator StoreLightingData(MagicLightmapSwitcher activeSwitcherInstance, string lightmapName, bool manualBaking = false)
        {
            ...
            StoreEnvironmentSettings storeEnvironmentSettings = new StoreEnvironmentSettings();
            ...
            if (File.Exists(sceneLightmapDataPath + "/" + SceneManager.GetActiveScene().name + "_" + lightmapName + ".asset"))
            {
                File.Delete(sceneLightmapDataPath + "/" + SceneManager.GetActiveScene().name + "_" + lightmapName + ".asset");
            }

            lightmapDataAsset = ScriptableObject.CreateInstance<StoredLightmapData>();
            AssetDatabase.CreateAsset(lightmapDataAsset, sceneLightmapDataPath + "/" + SceneManager.GetActiveScene().name + "_" + lightmapName + ".asset");            
            AssetDatabase.SaveAssets();

            yield return null;

            lightmapDataAsset.sceneGUID = SceneManager.GetActiveScene().GetHashCode().ToString();
            lightmapDataAsset.workflow = activeSwitcherInstance.workflow;
            lightmapDataAsset.dataPrefix = SceneManager.GetActiveScene().name;
            ...
            storingStages.Add(storeEnvironmentSettings.Execute(lightmapDataAsset, activeSwitcherInstance));
            ...
            Debug.LogFormat("<color=cyan>MLS:</color> Lighting Data for scene \"" + SceneManager.GetActiveScene().name + "\" stored successfully. " +
        ...
        public static void CreateLightmapScenario(MagicLightmapSwitcher lightmapSwitcher, string name, List<StoredLightmapData> storedLightmapData = null)
        {
            ...
            string prefix = SceneManager.GetActiveScene().name + "_";
            ...
            {
                storedLightingScenario = ScriptableObject.CreateInstance<StoredLightingScenario>();

                storedLightingScenario.workflow = lightmapSwitcher.workflow;
                storedLightingScenario.sourceObject = lightmapSwitcher.gameObject;
                storedLightingScenario.targetScene = SceneManager.GetActiveScene().name;
                ...
                lightmapSwitcher.UpdateStoredArray(SceneManager.GetActiveScene().name, true);
            }
        }
    }
}
