...
namespace MagicLightmapSwitcher
{
    ...
    {
        public static GUIStyle rightBoldLabelStyle;
        public static GUIStyle leftBoldLabelStyle;
        ...
        public static void InitStyles()
        {
            ...
            rightBoldLabelStyle = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleRight,
                margin = new RectOffset(),
                padding = new RectOffset(0, 0, 3, 0),
            ...
            leftBoldLabelStyle = new GUIStyle(GUI.skin.label)
            {
                alignment = TextAnchor.MiddleLeft,
                margin = new RectOffset(),
                padding = new RectOffset(0, 0, 3, 0),
        ...
        static public bool DrawHeader(string text, bool forceOn, Color backoundColor, List<StoredLightingScenario> storedLightmapScenarios, int scenarioIndex)
        {
            return DrawHeader(text, text, forceOn, backoundColor, storedLightmapScenarios, scenarioIndex);
        }
        ...
        static public bool DrawHeader(string text, string key, bool forceOn, Color backgroundColor, List<StoredLightingScenario> storedLightmapScenarios, int scenarioIndex)
        {
            ...
            using (new GUILayout.HorizontalScope())
            {
                ...
                if (MLSManager.renaming)
                {
                    if (GUILayout.Button("Rename"))
                    {
                        MLSManager.RenameStoredLightmapScenario(scenarioIndex);
                    }
                }
                ...
                if (GUILayout.Button("Remove"))
                {
                    if (EditorUtility.DisplayDialog(
                        ...
                        "Are you sure you want to delete the scenario? This operation is irreversible.",
                        "OK",
                        "Cancel"))
                    {
                        MLSManager.RemoveStoredLightingScenario(scenarioIndex);
                        return false;
                    }
                }
        ...
        public static void DrawBlendingConfigurator(MagicLightmapSwitcher activeSwitcherSource, StoredLightingScenario storedLightingScenario)
        {
            ...
            using (new GUILayout.VerticalScope(GUI.skin.box))
            {
                ...
                {
                    if (storedLightingScenario.globalBlendFactor == 0)
                    {
                        GUILayout.Label("START", MLSEditorUtils.rightBoldLabelStyle);
                    }
                    else if (storedLightingScenario.globalBlendFactor == 1)
                    {
                        GUILayout.Label("END", MLSEditorUtils.rightBoldLabelStyle);
                    } 
                    else
                    {
                        GUILayout.Label($"{storedLightingScenario.globalBlendFactor.ToString("F2")}", MLSEditorUtils.rightBoldLabelStyle);
                    }
                }
                ...
                if (storedLightingScenario.stopPreviewPlayback)
                {
                    ...
                    if (storedLightingScenario.prevBlendValue != storedLightingScenario.globalBlendFactor && !Application.isPlaying)
                    {
                        ...
                        Blending.Blend(activeSwitcherSource, storedLightingScenario.globalBlendFactor, storedLightingScenario);
                    }
                }
                else
                {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    for (int j = 0; j < storedLightingScenario.blendableLightmaps.Count; j++)
                    {
                        ...
                        if (j == 0)
                        {
                            labelRect = new Rect(globalBlendSlider.position.x + 1, globalBlendSlider.position.y + 25, 20, 20);

                            mark.position = new Vector2(
                                globalBlendSlider.position.x + 4.5f,
                        ...
                        {
                            labelRect = new Rect(globalBlendSlider.position.x + (globalBlendSlider.width - 20), globalBlendSlider.position.y + 25, 20, 20);

                            mark.position = new Vector2(
                            globalBlendSlider.position.x + (globalBlendSlider.width - 4.5f),
                            globalBlendSlider.position.y + 15);
                        ...
                        {
                            labelRect = new Rect(
                            globalBlendSlider.position.x + (globalBlendSlider.width * storedLightingScenario.blendableLightmaps[j].startValue) - 9,
                            globalBlendSlider.position.y + 25, 20, 20);
