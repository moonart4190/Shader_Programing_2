...
namespace MagicLightmapSwitcher
{
    public class MLSManager : EditorWindow
    {
        ...
        public static string focusedWindowId;
        ...
        private static List<string> storedScenariosToReplace = new List<string>();
        private static List<string> storedScenariosToLoad = new List<string>();
        private static List<string> storedScenarioPresets = new List<string>();
        ...
        private static Vector2 storedDataScrollPosition;
        ...
        [MenuItem("Tools/Magic Tools/Magic Lightmap Switcher/Clean Options/Clear All Data...", priority = 0)]
        ...
        public static void RemoveStoredLightingScenario(int removeIndex)
        {
            ...
            activeSwitcherInstance.onStartLightingScenario = 0;

            AssetDatabase.DeleteAsset(AssetDatabase.GetAssetPath(scenarioToRemove));
        }

        public static void RenameStoredLightmapScenario(int scenarioIndex)
        {
            renaming = false;
            _renamed = true;
            var targetScenario = activeSwitcherInstance.availableScenarios[scenarioIndex];
            var newName = targetScenario.prefix + targetScenario.scenarioName;

            AssetDatabase.RenameAsset(AssetDatabase.GetAssetPath(targetScenario), newName);
        ...
        public static void CheckState()
        {
            if (activeSwitcherInstance == null)
            {
                MagicLightmapSwitcher.SetBlendingOptionsGlobalStatic(MagicLightmapSwitcher.BlendingOptions.None);
            ...
            {
                activeSwitcherInstance.SetBlendingOptionsGlobal(MagicLightmapSwitcher.BlendingOptions.All);
            }
        ...
        void OnGUI()
        {
            ...
            storedScenariosToReplace.Clear();
            storedScenariosToReplace.Add("-None-");
            ...
            for (int i = 0; i < activeSwitcherInstance.availableScenarios.Count; i++)
            {
                if (activeSwitcherInstance.availableScenarios[i] != null)
                {
                    if (workflow == MagicLightmapSwitcher.Workflow.MultiScene)
                    {
                        ...
                        {
                            storedScenariosToReplace.Add(activeSwitcherInstance.availableScenarios[i].name);
                        }
                    }
                    else
                    {
            ...
            for (int i = 0; i < activeSwitcherInstance.availableScenarios.Count; i++)
            {
                if (activeSwitcherInstance.availableScenarios[i] != null)
                {
                    if (workflow == MagicLightmapSwitcher.Workflow.MultiScene)
                    {
                        if (activeSwitcherInstance.availableScenarios[i].name.Contains(EditorSceneManager.GetActiveScene().name))
                        {
                            storedScenariosToLoad.Add(activeSwitcherInstance.availableScenarios[i].name);
                        }
                    }
                    else
                    {
            ...
            if (activeSwitcherInstance.availableScenarios != null &&
                activeSwitcherInstance.availableScenarios.Count > 0 &&
                activeSwitcherInstance.availableScenarios[activeSwitcherInstance.onStartLightingScenario].blendableLightmaps != null)
            {
                storedScenarioPresets.Clear();

                for (int i = 0; i < activeSwitcherInstance.availableScenarios[activeSwitcherInstance.onStartLightingScenario].blendableLightmaps.Count; i++)
                {
                    if (activeSwitcherInstance.availableScenarios[activeSwitcherInstance.onStartLightingScenario].blendableLightmaps[i] != null)
                    {
                        if (activeSwitcherInstance.availableScenarios[activeSwitcherInstance.onStartLightingScenario].blendableLightmaps[i].lightingData != null)
                        {
                            storedScenarioPresets.Add(activeSwitcherInstance.availableScenarios[activeSwitcherInstance.onStartLightingScenario].blendableLightmaps[i].lightingData.name);
                        }
                    }
                }
            }
            ...
            selectedTab = GUILayout.Toolbar(selectedTab, new string[] { "Storing", "Switching/Blending", "Global Settings" });
            ...
            switch (selectedTab)
            {
                        ...
                        using (new GUILayout.VerticalScope(GUI.skin.box))
                        {
                            ...
                            using (new GUILayout.VerticalScope(GUI.skin.box))
                            {
                                ...
                                if (activeSwitcherInstance.lightingPresets.Count > 0)
                                {
                                    ...
                                    using (new GUILayout.HorizontalScope())
                                    {
                                        if (GUILayout.Button("Select All", GUILayout.MaxWidth(240)))
                                        {
                                            for (int i = 0; i < activeSwitcherInstance.lightingPresets.Count; i++)
                                            {
                                                activeSwitcherInstance.lightingPresets[i].included = true;
                                            }
                                        ...
                                        if (GUILayout.Button("Deselect All", GUILayout.MaxWidth(240)))
                                        {
                                            for (int i = 0; i < activeSwitcherInstance.lightingPresets.Count; i++)
                                            {
                                                activeSwitcherInstance.lightingPresets[i].included = false;
                                            }
                                        ...
                                        if (GUILayout.Button("Remove Selected", GUILayout.MaxWidth(240)))
                                        {
                                            if (EditorUtility.DisplayDialog("Magic Lightmap Switcher",
                                                        "Remove selected Presets?", "Yes", "Cancel"))
                                            {
                                                var tempList = new List<MagicLightmapSwitcher.SceneLightingPreset>();

                                                for (int i = 0; i < activeSwitcherInstance.lightingPresets.Count; i++)
                                                {
                                                    if (!activeSwitcherInstance.lightingPresets[i].included)
                                                    {
                                                        tempList.Add(activeSwitcherInstance.lightingPresets[i]);
                                                    }
                                                }

                                                activeSwitcherInstance.lightingPresets = tempList;
                                            }
                                ...
                                presetsScrollPosition = EditorGUILayout.BeginScrollView(presetsScrollPosition, GUILayout.MaxHeight(0));
                                        ...
                                        if (createLightmapScenario)
                                        {
                                            if (activeSwitcherInstance.availableScenarios.Count > 0)
                                            {
                                                using (new GUILayout.HorizontalScope())
                                                {
                                                    ...
                                                    scenarioToChange = EditorGUILayout.Popup(scenarioToChange, storedScenariosToReplace.ToArray(), GUILayout.MaxWidth(240));
                                                }
                                            }
                                        }
                    ...
                    using (new GUILayout.VerticalScope(GUI.skin.box))
                    {
                        ...
                        using (new GUILayout.VerticalScope(GUI.skin.box))
                        {
                            ...
                            if (currentStoredData != null && currentStoredData.Count > 0)
                            {
                                ...
                                storedDataScrollPosition = EditorGUILayout.BeginScrollView(storedDataScrollPosition);
                                ...
                                using (new GUILayout.VerticalScope(GUI.skin.box))
                                {
                                    for (int i = 0; i < currentStoredData.Count; i++)
                                    {
                                        ...
                                        {
                                            if (currentStoredData[i].name.Contains(SceneManager.GetSceneAt(selectedScene).name))
                                            {
                                                using (new GUILayout.HorizontalScope())
                                                {
                                                    GUILayout.Label(currentStoredData[i].name.ToString(), GUILayout.MinWidth(200));
                ...
                case 2:
                    ...
                    {
                        DrawGlobalSettingsSection(activeSwitcherInstance);
                    }
                    break;
            }
        }
        ...
        public static void DrawSwitchingBlendingSection(MagicLightmapSwitcher activeInstance)
        {
            ...
            using (new GUILayout.VerticalScope(GUI.skin.box))
            {
                using (new GUILayout.VerticalScope(GUI.skin.box))
                {
                    if (activeInstance.useTextureArrays)
                    {
                        if (activeSwitcherInstance.systemProperties.showShaderInitWarning)
                        {
                            EditorGUILayout.HelpBox("When you use Texture Arrays, " +
                                "there may be situations where after reloading the " +
                                "scene (or immediately after activating this option) " +
                                "your lightmaps may look broken or blending may not" +
                                " work correctly.  Sometimes it's enough to move the " +
                                "Global Branding slider to fix the situation. If the " +
                                "problem persists, then enable the \"CallLightmapBlending\" " +
                                "script and switch to Play Mode. This will trigger " +
                                "initialization of the shaders and the problem will be fixed.",
                                    MessageType.Info);
                            ...
                            using (new GUILayout.HorizontalScope())
                            {
                                ...
                                {
                                    activeSwitcherInstance.systemProperties.showShaderInitWarning = false;
                                }
                            }
                        }

                        if (!activeInstance.globalDataArraysInitialized)
                        {
                            EditorGUILayout.HelpBox("" +
                                "Global texture arrays have not been initialized yet. Click the button below to prepare the data.",
                                MessageType.Info);

                            if (GUILayout.Button("Reload Data"))
                            {
                                activeInstance.storedDataUpdatingProcess = false;
                                activeInstance.UpdateStoredArray(SceneManager.GetActiveScene().name, true);
                            }
                    ...
                    {
                        //if (activeInstance.storedDataUpdatingProcess)
                        //{
                        //    EditorGUILayout.HelpBox("Data Arrays initialization in progress. \r\n" +
                        //    "This should not take too long, if the process does not finish for a long time, " +
                        //    "then click the \"Reload Data\" button. If the process failed, then inform the developer about it.",
                        //        MessageType.Info);

                        //    if (GUILayout.Button("Reload Data"))
                        //    {
                        //        activeInstance.storedDataUpdatingProcess = false;
                        //        activeInstance.StartCoroutine(activeInstance.UpdateStoredArray(SceneManager.GetActiveScene().name, true));
                        //    }
                    ...
                    activeSwitcherInstance.availableScenarios.Count > 0)
                {
                    using (new GUILayout.VerticalScope(GUI.skin.box))
                    {
                        if (activeInstance.loadScenarioOnStart)
                        {
                            GUILayout.Label("Load Scenario On Start", MLSEditorUtils.captionStyle);

                            using (new GUILayout.HorizontalScope())
                            {
                                GUILayout.Label(MLSTooltipManager.MainComponent.GetParameter("Scenario To Load", MLSTooltipManager.MainComponent.Tabs.Storing), GUILayout.MinWidth(180));
                                activeInstance.onStartLightingScenario = EditorGUILayout.Popup(activeInstance.onStartLightingScenario, storedScenariosToLoad.ToArray(), GUILayout.MaxWidth(240));

                            }

                            using (new GUILayout.HorizontalScope())
                            {
                                if (storedScenarioPresets.Count > 0)
                                {
                                    GUILayout.Label(MLSTooltipManager.MainComponent.GetParameter("Preset To Load", MLSTooltipManager.MainComponent.Tabs.Storing), GUILayout.MinWidth(180));
                                    activeInstance.onStartPreset = EditorGUILayout.Popup(activeInstance.onStartPreset, storedScenarioPresets.ToArray(), GUILayout.MaxWidth(240));
                                }
                                else
                                {
                                    GUILayout.Label("No Presets in selected Scenario", GUILayout.MaxWidth(240));
                                }
                            }
                ...
                using (new GUILayout.VerticalScope(GUI.skin.box))
                {
                    ...
                    if (scenarios != null && scenarios.Count > 0)
                    {
                        ...
                        for (int i = 0; i < scenarios.Count; i++)
                        {
                            ...
                            {
                                targetScenario = scenarios[i].scenarioName;
                                ...
                                using (new GUILayout.VerticalScope(GUI.skin.box))
                                {
                                    using (new GUILayout.HorizontalScope())
                                    {
                                        ...
                                        {
                                            var path = AssetDatabase.GetAssetPath(scenarios[i]);
                                            var fullName = path.Split("/").Last();
                                            var currentName = fullName.Split(scenarios[i].prefix)[1].Split(".")[0];

                                            if (scenarios[i].scenarioName != currentName)
                                            {
                                                if (!_renamed)
                                                {
                                                    renaming = true;
                                    ...
                                    scenarios[i].reorderableStoredLightmapsData.DoLayoutList();
                                    scenarios[i].scenarioSerializedObject.ApplyModifiedProperties();

                                    if (!scenarios[i].selfTestSuccess)
                                    {
                                        var selfTestResult = scenarios[i].SelfTest();

                                        if (selfTestResult != "")
                                        {
                                            EditorGUILayout.TextArea(selfTestResult, MLSEditorUtils.helpBoxStyle);
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        using (new GUILayout.VerticalScope(GUI.skin.box))
                                        {
                                            if (scenarios[i].reorderableStoredLightmapsData.count > 0)
                                            {
        ...
        private static void DrawGlobalSettingsSection(MagicLightmapSwitcher activeInstance)
        {
            if (MLSLightmapDataStoring.bakingLightingData)
            {
                GUI.enabled = false;
            ...
            using (new GUILayout.VerticalScope(GUI.skin.box))
            {
                if (SystemInfo.supports2DArrayTextures && SystemInfo.supportsCubemapArrayTextures)
                {                    
                    using (new GUILayout.HorizontalScope())
                    {
                        GUILayout.Label(
                            MLSTooltipManager.MainComponent.GetParameter("Use Texture Arrays",
                                MLSTooltipManager.MainComponent.Tabs.Storing),
                            GUILayout.MinWidth(200));
                        activeInstance.useTextureArrays =
                            GUILayout.Toggle(activeInstance.useTextureArrays, "", GUILayout.MaxWidth(250));
                    }

                    if (!activeInstance.useTextureArrays)
                    {
                        EditorGUILayout.HelpBox(
                            "Your platform supports Texture2D and Texture Cube arrays. " +
                            "It is recommended to enable this option, as it significantly " +
                            "increases FPS and saves the Batcher's work.", MessageType.Info);
                    }
                    else
                    {
                        Shader.EnableKeyword("MLS_TEXTURE_ARRAYS_ON");
                ...
                else
                {
                    if (activeInstance.useTextureArrays)
                    {
                        activeInstance.useTextureArrays = false;
                ...
                {
                    GUILayout.Label(
                        MLSTooltipManager.MainComponent.GetParameter("Ignore Main Camera Warning",
                        MLSTooltipManager.MainComponent.Tabs.Storing),
                        GUILayout.MinWidth(200));
                    activeInstance.systemProperties.ignoreCameraError =
                        GUILayout.Toggle(activeInstance.systemProperties.ignoreCameraError, "", GUILayout.MaxWidth(250));
                }

                using (new GUILayout.HorizontalScope())
                {
                    GUILayout.Label(
                        MLSTooltipManager.MainComponent.GetParameter("Load Scenario On Start",
                        MLSTooltipManager.MainComponent.Tabs.Storing),
                        GUILayout.MinWidth(200));

                    activeInstance.loadScenarioOnStart = GUILayout.Toggle(activeInstance.loadScenarioOnStart, "", GUILayout.MaxWidth(250));
                }
            }
        }
