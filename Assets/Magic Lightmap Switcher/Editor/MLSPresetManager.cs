...
using static MagicLightmapSwitcher.MagicLightmapSwitcher.SceneLightingPreset.GameObjectSettings;
...
namespace MagicLightmapSwitcher
{
    ...
    public class MLSPropertiesSelectionWindow : EditorWindow
    {
        public static MLSPropertiesSelectionWindow propertiesSelectionManagerWindow;
        public static List<ObjectShaderProperty> propertiesList = new List<ObjectShaderProperty>();
        private static Vector2 _scrollPos;

        public static void Init(List<ObjectShaderProperty> inProperties)
        {
            propertiesList = inProperties;

            propertiesSelectionManagerWindow = ScriptableObject.CreateInstance<MLSPropertiesSelectionWindow>();
            propertiesSelectionManagerWindow.titleContent = new GUIContent("Select Properies...");
            propertiesSelectionManagerWindow.minSize = new Vector2(350 * EditorGUIUtility.pixelsPerPoint, 250 * EditorGUIUtility.pixelsPerPoint);
            propertiesSelectionManagerWindow.maxSize = new Vector2(350 * EditorGUIUtility.pixelsPerPoint, 250 * EditorGUIUtility.pixelsPerPoint);
            propertiesSelectionManagerWindow.ShowAuxWindow();
        }
        ...
        void OnGUI()
        {
            ...
            using (new GUILayout.VerticalScope(GUI.skin.box))
            {
                for (int i = 0; i < propertiesList.Count; i++)
                {
                    ...
                    {
                        GUILayout.Label(propertiesList[i].name, GUILayout.Width(200));

                        GUILayout.FlexibleSpace();

                        if (!propertiesList[i].trackable)
                        {
                            if (GUILayout.Button("Add", GUILayout.Width(80)))
                            {
                                propertiesList[i].trackable = true;
                            }
                        }
                        else
                        {
                            ...
                            {
                                propertiesList[i].trackable = false;
                            }
                        }
                    }
                }
    ...
    public class MLSPresetManager : EditorWindow
    {
        ...
        private static Material _currentSkyboxMaterial;
        private static Material _lastSkyboxMaterial;
        private static bool _externalChange;

        public static void Init(bool forceActiveScene = false)
        {
            presetsManagerWindow = ScriptableObject.CreateInstance<MLSPresetManager>();
            presetsManagerWindow.titleContent = new GUIContent("Preset Manager");
            ...
            presetsManagerWindow.ShowUtility();
        ...
        public static void CreateNewPreset(MagicLightmapSwitcher magicLightmapSwitcher, MagicLightmapSwitcher.SceneLightingPreset copyFrom = null)
        {
            ...
            if (magicLightmapSwitcher.lightingPresets.Count > 0)
            {
                if (copyFrom != null)
                {
                    lightingPreset.lightSourceSettings = copyFrom.lightSourceSettings.Select(element => new MagicLightmapSwitcher.SceneLightingPreset.LightSourceSettings() 
                    {
                        ...
                        lightCookie = element.lightCookie,
                    ...
                    lightingPreset.gameObjectsSettings = 
                        copyFrom.gameObjectsSettings.Select
                        (element => new MagicLightmapSwitcher.SceneLightingPreset.GameObjectSettings()
                        {
                            objectShaderProperties = element.objectShaderProperties.Select(el => new ObjectShaderProperty()
                            {
                                trackable = el.trackable,
                                name = el.name,
                                type = el.type,
                                floatValue = el.floatValue,
                                colorValue = el.colorValue,
                                textureValue = el.textureValue,
                                min = el.min,
                                max = el.max,
                            }).ToList(),

                            currentMaterial = element.currentMaterial,
                            lastMaterial = element.lastMaterial,
                            meshRenderer = element.meshRenderer,
                            gameObject = element.gameObject,
                            instanceId = element.instanceId,
                            enabled = element.enabled,
                            justAdded = element.justAdded,
                            position = element.position,
                            rotation = element.rotation,
                            tempRotation = element.tempRotation,
                            globalFoldoutEnabled = element.globalFoldoutEnabled,
                            transformFoldoutEnabled = element.transformFoldoutEnabled,
                            materialFoldoutEnabled = element.materialFoldoutEnabled
                        }).ToList();
            ...
            else
            {
                ...
                {
                    lightingPreset.skyboxSettings.bakerySkyLightsSettings = new MagicLightmapSwitcher.SceneLightingPreset.SkyboxSettings.BakerySkyLightsSettings(bakerySkyLight);
                }
        ...
        private void AddLightToPreset(List<MagicLightmapSwitcher.SceneLightingPreset> presets, Light light)
        {
            ...
            for (int i = 0; i < presets.Count; i++)
            {
                ...
                {
                    EditorUtility.DisplayDialog("Magic Lightmap Switcher", "This Light already added to preset.", "OK");
                    return;
                }
                ...
                lightSourceSettings.lightCookie = light.cookie;
        ...
        private void AddBakeryLightToPreset<T>(MagicLightmapSwitcher.SceneLightingPreset preset, T light)
        {
            ...
            else if (bakerySkyLight != null)
            {
                ...
                {
                    preset.skyboxSettings.bakerySkyLightsSettings = new MagicLightmapSwitcher.SceneLightingPreset.SkyboxSettings.BakerySkyLightsSettings(bakerySkyLight);
                }
            }
        }
#endif
        ...
        private void AddCustomBlendableToPreset(List<MagicLightmapSwitcher.SceneLightingPreset> presets, MLSCustomBlendable customBlendable)
        {
            for (int i = 0; i < presets.Count; i++)
            {
                ...
                for (int j = 0; j < customBlendable.blendableCubemapFields.Count; j++)
                {
                    customBlendablesSettings.blendableCubemapParameters.Add(customBlendable.blendableCubemapFields[j].Name);
                    customBlendablesSettings.blendableCubemapParametersValues.Add((Cubemap) customBlendable.blendableCubemapFields[j].GetValue(customBlendable));
                }

                for (int j = 0; j < customBlendable.blendableColorFields.Count; j++)
                {
                    customBlendablesSettings.blendableColorParameters.Add(customBlendable.blendableColorFields[j].Name);
                    customBlendablesSettings.blendableColorParametersValues.Add((Color) customBlendable.blendableColorFields[j].GetValue(customBlendable));
                }

                for (int j = 0; j < customBlendable.blendableBoolFields.Count; j++)
                {
                    customBlendablesSettings.blendableBoolParameters.Add(customBlendable.blendableBoolFields[j].Name);
                    customBlendablesSettings.blendableBoolParametersValues.Add((bool) customBlendable.blendableBoolFields[j].GetValue(customBlendable));
                }
        ...
        void OnLostFocus()
        {
            if (!_externalChange &&
                ...
                (mouseOverWindow != null && mouseOverWindow.GetType().ToString() == "UnityEditor.ColorPicker"))
            {
                        ...
                        if (magicLightmapSwitcher.lightingPresets[lp].foldoutEnabled)
                        {
                                ...
                                for (int i = 0; i < magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings.Count; i++)
                                {
                                    ...
                                    using (new GUILayout.VerticalScope(GUI.skin.box))
                                    {
                                        using (new GUILayout.HorizontalScope())
                                        {
                                            ...
                                            {
                                                Selection.activeGameObject = magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject;
                                            }
                                        ...
                                        if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].globalFoldoutEnabled)
                                        {
                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].transformFoldoutEnabled =
                                                    EditorGUILayout.Foldout(
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].transformFoldoutEnabled, "Transform", true);
                                                ...
                                                if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].justAdded)
                                                {
                                                    ...
                                                    magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].tempRotation =
                                                        TransformUtils.GetInspectorRotation(
                                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject.transform);
                                                }
                                            ...
                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].materialFoldoutEnabled =
                                                EditorGUILayout.Foldout(
                                                    magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].materialFoldoutEnabled, "Material Properies", true);

                                            if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].materialFoldoutEnabled)
                                            {
                                                if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject.GetComponent<MeshRenderer>() == null)
                                                {
                                                    EditorGUILayout.HelpBox(
                                                        "The object does not have a Mesh Renderer component.", MessageType.Info);
                                                    continue;
                                                }

                                                magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].lastMaterial =
                                                magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject.GetComponent<MeshRenderer>().sharedMaterial;

                                                if (!MagicLightmapSwitcher.CheckIfStatic(magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject))
                                                {
                                                    EditorGUILayout.HelpBox("" +
                                                        "Some properties of a non-static object may not affect the baking results, " +
                                                        "such as the emission color.", MessageType.Info);
                                                }                                                
                                                
                                                if (AssetDatabase.GetAssetPath(magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].lastMaterial).StartsWith("Packages"))
                                                {
                                                    EditorGUILayout.HelpBox("" +
                                                        "The default material is used on the object. " +
                                                        "MLS will not be able to control the parameters of such material, create a separate material " +
                                                        "for the object or use an existing one from the project assets.", MessageType.Info);
                                                }
                                                else
                                                {
                                                    if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].meshRenderer == null ||
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].propertyBlock == null)
                                                    {
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].meshRenderer =
                                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject.GetComponent<MeshRenderer>();
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].propertyBlock = new MaterialPropertyBlock();
                                                    }

                                                    if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].propertyBlock == null)
                                                    {
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].gameObject.GetComponent<MeshRenderer>().GetPropertyBlock(
                                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].propertyBlock);
                                                    }

                                                    if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].lastMaterial != null)
                                                    {
                                                        if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties == null ||
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties.Count == 0 ||
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].currentMaterial !=
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].lastMaterial)
                                                        { 
                                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].currentMaterial =
                                                                magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].lastMaterial;

                                                            magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties =
                                                                new List<ObjectShaderProperty>();

                                                            for (int p = 0; p < magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].currentMaterial.shader.GetPropertyCount(); p++)
                                                            {
                                                                magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties.Add(new ObjectShaderProperty());
                                                                magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties[p].Create(
                                                                    magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].currentMaterial,
                                                                    magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].currentMaterial.shader, p);
                                                            }
                                                        }

                                                        using (new GUILayout.HorizontalScope())
                                                        {
                                                            if (GUILayout.Button("Select Properties"))
                                                            {
                                                                MLSPropertiesSelectionWindow.Init(magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties);
                                                            }
                                                        }

                                                        var trackableShaderProperties = 0;

                                                        for (int p = 0; p < magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].currentMaterial.shader.GetPropertyCount(); p++)
                                                        {
                                                            if (magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties[p].trackable)
                                                            { 
                                                                var currentProperty = magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties[p];
                                                                    ...
                                                                    switch (currentProperty.type)
                                                                    {
                                                                        case ShaderPropertyType.Float:
                                                                            currentProperty.floatValue =
                                                                                EditorGUILayout.FloatField(currentProperty.name, currentProperty.floatValue);
                                                                            break;
                                                                        case ShaderPropertyType.Color:
                                                                            currentProperty.colorValue =
                                                                                EditorGUILayout.ColorField(new GUIContent(currentProperty.name), currentProperty.colorValue, true, true, true);
                                                                            break;
                                                                        case ShaderPropertyType.Range:
                                                                            currentProperty.floatValue =
                                                                                EditorGUILayout.Slider(currentProperty.name, currentProperty.floatValue, currentProperty.min, currentProperty.max);
                                                                            break;
                                                                        case ShaderPropertyType.Texture:
                                                                            currentProperty.textureValue =
                                                                                EditorGUILayout.ObjectField(currentProperty.name, currentProperty.textureValue, typeof(Cubemap), false) as Cubemap;
                                                                            break;
                                                                    }
                                                                }

                                                                trackableShaderProperties++;
                                                            }
                                                        }

                                                        var mlsObject = magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].
                                                                gameObject.GetComponent<MLSObject>();

                                                        if (mlsObject == null)
                                                        {
                                                            mlsObject = magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].
                                                                gameObject.AddComponent<MLSObject>();
                                                        }

                                                        if (trackableShaderProperties > 0) 
                                                        {
                                                            mlsObject.haveTrackableShaderProps = true;
                                                        }
                                                        else
                                                        {
                                                            mlsObject.haveTrackableShaderProps = false;
                                                        }
                                                    }
                                                    else
                                                    {
                                                        magicLightmapSwitcher.lightingPresets[lp].gameObjectsSettings[i].objectShaderProperties = null;
                                                        EditorGUILayout.HelpBox("There is no assigned Material in the Object Mesh Renderer.", MessageType.Info);
                                                    }
                                                }
                                            }
                                        }
                                                ...
                                                if (magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].settingsFoldoutEnabled)
                                                {
...
#endif
                                                    {
                                                        ...
                                                        case LightType.Directional:
                                                            ...
                                                            //magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].lightCookie =
                                                            //    EditorGUILayout.ObjectField("Light Cookie", magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].lightCookie, typeof(Texture), false) as Texture;

                                                            //magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].lightCookieSize =
                                                            //    EditorGUILayout.Vector2Field("Cookie Size", magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].lightCookieSize);

                                                            //magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].lightCookieOffset =
                                                            //    EditorGUILayout.Vector2Field("Cookie Offset", magicLightmapSwitcher.lightingPresets[lp].lightSourceSettings[i].lightCookieSize);
#endif
                                                            GUI.enabled = true;
                                                            break;
#if MT_HDRP_7_INCLUDED || MT_HDRP_8_INCLUDED || MT_HDRP_9_INCLUDED || MT_HDRP_10_INCLUDED || MT_HDRP_11_INCLUDED || MT_HDRP_12_INCLUDED || MT_HDRP_13_INCLUDED || MT_HDRP_14_INCLUDED
                            ...
                            using (new GUILayout.VerticalScope(GUI.skin.box))
                            {
                                ...
                                if (magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.globalFoldoutEnabled)
                                {
                                    using (new GUILayout.VerticalScope(GUI.skin.box))
                                    {
                                        if (RenderSettings.skybox != null)
                                        {
                                            ...
                                            if (magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties == null ||
                                            magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties.Count == 0 ||
                                            _currentSkyboxMaterial != _lastSkyboxMaterial)
                                            {
                                                _lastSkyboxMaterial = _currentSkyboxMaterial;
                                                magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties =
                                                    new List<MagicLightmapSwitcher.SceneLightingPreset.SkyboxSettings.SkyboxShaderProperty>();

                                                for (int i = 0; i < RenderSettings.skybox.shader.GetPropertyCount(); i++)
                                                {
                                                    var properties = new MagicLightmapSwitcher.SceneLightingPreset.SkyboxSettings.SkyboxShaderProperty();

                                                    properties.Create(RenderSettings.skybox, RenderSettings.skybox.shader, i);
                                                    magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties.Add(properties);
                                                }
                                            }

                                            bool isPanoramic = false;

                                            if (magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties.Find(item => item.name == "_Layout") != null)
                                            {
                                                isPanoramic = true;
                                            }
                                            ...
                                            for (int i = 0; i < RenderSettings.skybox.shader.GetPropertyCount(); i++)
                                            {
                                                var currentProperty = magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties[i];

                                                using (new GUILayout.HorizontalScope(GUI.skin.box))
                                                {
                                                    currentProperty.trackable = EditorGUILayout.Toggle(currentProperty.trackable, GUILayout.MaxWidth(20));

                                                    GUI.enabled = currentProperty.trackable;
                                                    ...
                                                    switch (currentProperty.type)
                                                    {
                                                        ...
                                                        case ShaderPropertyType.Color:
                                                            currentProperty.colorValue =
                                                                EditorGUILayout.ColorField(currentProperty.name, currentProperty.colorValue);
                                                            break;
                                                        case ShaderPropertyType.Range:
                                                            currentProperty.floatValue =
                                                                EditorGUILayout.Slider(currentProperty.name, currentProperty.floatValue, currentProperty.min, currentProperty.max);
                                                        ...
                                                        case ShaderPropertyType.Texture:
                                                            ...
                                                            {
                                                                currentProperty.textureValue =
                                                                    EditorGUILayout.ObjectField(currentProperty.name, currentProperty.textureValue, typeof(Texture2D), false) as Texture2D;
                                                            }
                                                            else
                                                            {
                                                                currentProperty.textureValue =
                                        ...
                                        {
                                            magicLightmapSwitcher.lightingPresets[lp].skyboxSettings.skyboxShaderProperties = null;
                                            EditorGUILayout.HelpBox("There is no assigned Skybox Material in the Environment settings.", MessageType.Info);
                                        }
                                ...
                                if (magicLightmapSwitcher.lightingPresets[lp].environmentSettings.globalFoldoutEnabled)
                                {
                                    using (new GUILayout.VerticalScope(GUI.skin.box))
                                    {
										magicLightmapSwitcher.lightingPresets[lp].environmentSettings.sun =
                                           EditorGUILayout.ObjectField("Sun Source", magicLightmapSwitcher.lightingPresets[lp].environmentSettings.sun, typeof(Light), true) as Light;
